...
const niteChallenges = [
  { title: "Graph Grief", flag: "nite{Th3_Qu4ntum_****}", type: "web", slug: "graph-grief" },
  { title: "Database Reincursion", flag: "nite{neVer_90nn4_****}", type: "web", slug: "database-reincursion" },
];
...
const NiteCTF = () => {
  ...
  return (
    <div className="min-h-screen relative grid-bg scanlines page-aurora">
      ...
      <TopNav backTo="/" title="NiteCTF 2025" subtitle="ctf writeups" />
          ...
          <aside className="lg:col-span-4 space-y-4 lg:sticky lg:top-24">
            <div className="bg-card/50 glass border border-border/60 rounded-2xl p-5">
              ...
              <div className="rounded-2xl border border-border/60 bg-background/20 p-4">
                <div className="text-lg font-display text-primary leading-none">NiteCTF 2025</div>
                  <div className="text-[10px] font-mono text-muted-foreground/70 uppercase tracking-[0.22em] mt-2">{niteChallenges.length} solves • web</div>
              </div>

              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className="rounded-xl border border-border/60 bg-background/20 p-3">
                  <div className="text-2xl font-display text-primary leading-none">{niteChallenges.length}</div>
                  <div className="text-[10px] font-mono uppercase tracking-[0.22em] text-muted-foreground mt-1">writeups</div>
                </div>
                <div className="rounded-xl border border-border/60 bg-background/20 p-3">
                  <div className="text-2xl font-display text-primary leading-none">{new Set(niteChallenges.map((c) => c.type)).size}</div>
              ...
              <div className="mt-4 rounded-2xl border border-border/60 bg-background/20 p-4">
                ...
                {(Object.keys(tagConfig) as Array<keyof typeof tagConfig>).map((t) => (
                  <div key={t} className="flex items-center gap-3 py-1">
                    ...
                    <span className="ml-auto text-xs font-mono text-primary/80">{niteChallenges.filter((c) => c.type === t).length}</span>
                  </div>
                ))}
              </div>
            </div>
